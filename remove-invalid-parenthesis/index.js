...
var removeInvalidParentheses = function (s) {
  ...
  function helper(index, leftCount, rightCount, numLeftToRemove, numRightToRemove, prefix) {
    ...
    if (c === '(' && numLeftToRemove) {
      helper(index + 1, leftCount, rightCount, numLeftToRemove - 1, numRightToRemove, prefix);
    } else if (c === ')' && numRightToRemove) {
      //try to discard the ')'
      helper(index + 1, leftCount, rightCount, numLeftToRemove, numRightToRemove - 1, prefix);
    }
    ...
    if (c !== '(' && c !== ')') {
      helper(index + 1, leftCount, rightCount, numLeftToRemove, numRightToRemove, prefix);
      return;
    }

    //didn't remove it.  try using it
    if (c === '(') {
      helper(index + 1, leftCount + 1, rightCount, numLeftToRemove, numRightToRemove, prefix);
      return;
    }

    // c is a ')' && we still need to use right parens to balance
    if (rightCount < leftCount) {
      helper(index + 1, leftCount, rightCount + 1, numLeftToRemove, numRightToRemove, prefix);
    }
  }
};
